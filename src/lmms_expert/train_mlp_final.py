...
# Add parent directory to path to import coral utilities
...
FEATURES_FILE = os.path.join(SCRIPT_DIR, 'lmms_context_features.pt')
MODEL_SAVE_PATH = os.path.join(SCRIPT_DIR, 'lmms_coral_model.pth')
DEV_JSON = os.path.join(BASE_DIR, 'ambistory-main/dev.json')
...
def run_epoch(model, loader, optimizer, device, is_training=True):
    ...
    with context_manager:
        for x, y in loader:
            ...
            # Use imported loss - shift y to 0-based
...
def main():
    ...
    print(f"Training CORAL Model on {device}")
