...
def crossfit_grid_search(
    ...
    fit_fold: Callable[..., Tuple[Any, Dict[str, Any], Dict[str, Any]]],
    predict_fold: Callable[[Any, np.ndarray, Dict[str, Any]], np.ndarray],
    log_params: Optional[Callable[[Dict[str, Any]], Dict[str, Any]]] = None,
    on_config_start: Optional[Callable[[Dict[str, Any]], None]] = None,
    on_config_end: Optional[Callable[[Dict[str, Any], float, float], None]] = None,
    ...
    for cfg in configs:
        ...
        for fold_idx, val_idx in enumerate(folds):
            ...
            oof[val_idx] = predict_fold(model, x_train[val_idx], cfg).astype(np.float32)
            dev_sum += predict_fold(model, x_dev, cfg).astype(np.float32)
            test_sum += predict_fold(model, x_test, cfg).astype(np.float32)
            fold_info.append(info)
            fold_extra.append(extra)

        metric_result = metrics(oof, y_train, num_classes=num_classes, choices=train_choices)
        ...
        row = (log_params(cfg) if log_params else dict(cfg))
        row.update({"objective": float(objective_value), "metrics": metric_result})
        ...
        if on_config_end:
            on_config_end(cfg, float(objective_value), float(best_score))

    if best_payload is None:
        raise RuntimeError("Cross-fit search failed to produce a model.")
    ...
    dev_preds[np.where(train_mask)[0]] = best_payload["oof"]
    return dev_preds, test_bagged.astype(np.float32), {
        ...
        "fold_info": best_payload["fold_info"],
        "fold_extra": best_payload["fold_extra"],
