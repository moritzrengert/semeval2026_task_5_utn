...
def build_space(expert: str) -> List[Dict[str, any]]:
    """Return a list of hyperparameter dicts for the given expert."""
    common = {
        "lr": [7e-5, 1e-4, 2e-4],
        "weight_decay": [0.01, 0.03, 0.05],
        "dropout": [0.1, 0.2, 0.3],
        "hidden_dim": [128, 256],
        "warmup_ratio": [0.04, 0.06],
        "train_encoder_layers": [0, 1],
        "mse_weight": [0.0, 0.2],
        "soft_weight": [0.0, 0.5],
        "projector_dim": [0, 256],
    }
    if expert == "nli":
        common["pooling"] = ["cls", "mean"]
    return cartesian(common)
...
def run_trial(idx: int, expert: str, cfg: Dict[str, any], base_args: argparse.Namespace, out_dir: Path) -> Dict[str, any]:
    ...
    # Optional fields
    if cfg.get("projector_dim", 0) > 0:
        cmd += ["--hidden-dim", str(cfg["hidden_dim"])]
    if expert == "nli":
...
def main():
    ...
    with csv_path.open("w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["trial", "expert", "cfg", "dev_line", "status", "seconds", "log", "ckpt"]
        )
        writer.writeheader()
